...
(function(){
  ...
  async function compatOpenPreview(iid,mode='SP'){
    ...
    const copy=['QUOTE #'+first.quote,'Date: '+first.date,'Customer: '+(first.company_name||''),'Address: '+(first.address||''),'Services: '+services,'',...lines.map(z=>z.product+', '+z.qty+(z.platform?' '+z.platform:'')+' lines.map(z=>'<tr><td style="padding:4px 6px;font-weight:700;border:0">'+esc(z.product)+',</td><td style="padding:4px 6px;white-space:nowrap;border:0">'+esc(z.qty)+'</td><td style="padding:4px 6px;font-weight:700;white-space:nowrap;border:0">'+esc(z.platform)+'</td><td style="padding:4px 6px;font-weight:700;white-space:nowrap;border:0">$ '+Number(z.price||0).toFixed(costMode?2:0)+'</td><td style="padding:4px 6px;border:0;color:#777">('+esc(z.config)+')</td><td style="padding:4px 6px;white-space:nowrap;border:0">'+esc(z.transport)+'</td></tr>').join('');
  ...
  wireCurrent();
})();+Number(z.price||0).toFixed(costMode?2:0)+' ('+z.config+') '+z.transport)].join(String.fromCharCode(10));
